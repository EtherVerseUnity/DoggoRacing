...
public static class MalbersAnimationsExtensions
{
    ...
    {
        if (c == null || string.IsNullOrEmpty(component)) return null;

        var typ = Type.GetType(component);

        if (typ == null) return null;


        var sender = c.GetComponent(typ);
        if (sender == null) sender = c.GetComponentInParent(typ);
        if (sender == null) sender = c.GetComponentInChildren(typ);
